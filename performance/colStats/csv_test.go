...
func TestCSV2Float(t *testing.T) {
	// raw string backtick literal operator creates a multiline string
...
`
	...
	testCases := []struct {
		...
		},
		{
			...
			r:      iotest.TimeoutReader(bytes.NewReader([]byte{0})), // simulate a reading failure
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expErr != nil {
				...
				// using error values instead of comparing error strings makes the tests more resilient
				// to failures when the underlying message changes
				// for more complex scenarious, better to use custom error types instead
			...
			// check each element of the expected float64 slice
