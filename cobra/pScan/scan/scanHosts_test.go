// We use a different package to ensure we are testing the exposed (public)
// API of the scan package only
package scan_test
...
func TestRunHostFound(t *testing.T) {
	...
	for _, tc := range testCases {
		// Use port 0 to ensure it picks an available port on the machine tests are
		// running from
		...
		// Extract port used from the listener address
		...
		if tc.name == "ClosedPort" {
			// Close the port immediately after opening it, to ensure we are using
			// an available port, and that it is closed for the test
			ln.Close()
		}
	}
