...
// However, in tests we use a different package so the tests access only the exported
// types, variables and functions from the package being tested. This ensures the tests
// access the exposed objects the same as a user would do.
...
func TestDelete(t *testing.T) {
	...
	if l[1].Task != tasks[2] {
		t.Errorf("Expected %q, got %q isntead.", tasks[2], l[1].Task)
	}
}
